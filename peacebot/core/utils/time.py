...
class TimeMultConverter(lightbulb.converters.BaseConverter):
    async def convert(self, _: lightbulb.context.Context, mult: str) -> float:
        try:
            mult = float(mult)
        except ValueError:
            raise TimeError("Argument has to be float.")

        if mult < 1.0:
            raise TimeError("Unit must be more than 1.")

        return mult


class TimeDeltaConverter(lightbulb.converters.BaseConverter):
    async def convert(self, _: lightbulb.context.Context, unit: str):
        unit = unit.lower()
        ...
        if unit in ("s", "sec", "secs", "second", "seconds"):
            ...
            raise TimeError("Unknown time type.")


class TimeConverter(lightbulb.converters.BaseConverter):
    async def convert(self, _: lightbulb.context.Context, argument: str) -> float:
